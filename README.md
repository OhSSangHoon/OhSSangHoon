<h1 align="center">사용자 경험을 중심으로 생각하고 행동하는 개발자, 오상훈입니다 👋</h1>

<br>

<p align="center">
  기획부터 배포까지 전 과정에 직접 참여하며, 실제 사용자에게 의미 있는 경험을 만드는 것을 목표로 합니다.
</p>

<br>

---

<h3>🛠 Tech Stack</h3>
<div>
  <!-- Language -->
  <img src="https://img.shields.io/badge/JavaScript-FFE57A?style=for-the-badge&logo=javascript&logoColor=666633" height="25">
  <img src="https://img.shields.io/badge/TypeScript-90CAF9?style=for-the-badge&logo=typescript&logoColor=white" height="25">
  <img src="https://img.shields.io/badge/Java-FFCC80?style=for-the-badge&logo=openjdk&logoColor=7A5000" height="25">
  <br>
  <!-- Framework -->
  <img src="https://img.shields.io/badge/React-CE93D8?style=for-the-badge&logo=react&logoColor=white" height="25">
  <img src="https://img.shields.io/badge/React_Native-F48FB1?style=for-the-badge&logo=react&logoColor=white" height="25">
  <img src="https://img.shields.io/badge/Next.js-B0BEC5?style=for-the-badge&logo=nextdotjs&logoColor=white" height="25">
  <br>
  <!-- Styling / State -->
  <img src="https://img.shields.io/badge/Tailwind_CSS-B2EBF2?style=for-the-badge&logo=tailwindcss&logoColor=2A7A90" height="25">
  <img src="https://img.shields.io/badge/Zustand-BCAAA4?style=for-the-badge&logo=react&logoColor=white" height="25">
  <img src="https://img.shields.io/badge/React_Query-EF9A9A?style=for-the-badge&logo=reactquery&logoColor=white" height="25">
  <br>
  <!-- Infra -->
  <img src="https://img.shields.io/badge/Vercel-CFD8DC?style=for-the-badge&logo=vercel&logoColor=555555" height="25">
</div>
<br>

---

<h3>🚀 Projects</h3>

<table>
  <tr>
    <td width="50%" valign="top">
      <h4>🐝 <a href="http://webeelab.com">Webee</a> <sub>📱 Mobile App</sub></h4>
      <p>양봉 농가를 위한 스마트 통합 플랫폼<br>
      AI 질병 진단, 맞춤 수정벌 추천, 농가 간 거래까지 양봉에 필요한 모든 것을 하나의 앱에서 제공합니다.</p>
      <img src="https://img.shields.io/badge/React_Native-61DAFB?style=flat-square&logo=react&logoColor=black">
      <img src="https://img.shields.io/badge/Expo-000000?style=flat-square&logo=expo&logoColor=white">
      <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white">
      <img src="https://img.shields.io/badge/NativeWind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white">
      <img src="https://img.shields.io/badge/Zustand-433E38?style=flat-square&logo=react&logoColor=white">
    </td>
    <td width="50%" valign="top">
      <h4>🤝 <a href="https://meet-meet-psi.vercel.app/">Meet Meet</a></h4>
      <p>서울 2030을 위한 소셜 모임 매칭 플랫폼<br>
      취미·관심사 기반으로 모임을 찾고, 가입하고, 리뷰까지 남길 수 있는 서비스입니다.</p>
      <img src="https://img.shields.io/badge/Next.js_14-000000?style=flat-square&logo=nextdotjs&logoColor=white">
      <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white">
      <img src="https://img.shields.io/badge/React_Query-FF4154?style=flat-square&logo=reactquery&logoColor=white">
      <img src="https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white">
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4>🎵 Sentifl</h4>
      <p>감정 분석 기반 음악 추천 다이어리<br>
      오늘의 감정을 글로 기록하면, 그 감정에 맞는 음악을 추천해주는 웹 서비스입니다.</p>
      <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black">
      <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white">
      <img src="https://img.shields.io/badge/AWS_S3-FF9900?style=flat-square&logo=amazons3&logoColor=white">
    </td>
    <td width="50%" valign="top">
    </td>
  </tr>
</table>

<br>

---

<h3>🏆 Awards & Activities</h3>

| 연도 | 내용 |
|------|------|
| 2026.01 | 부울경 창업 부트캠프 **3등** |
| 2025.12 | 멋쟁이사자처럼 창업트랙 **특별상** |
| 2025.11 | AI 창업경진대회 **최우수상** |
| 2025.09 | 농림축산식품부 공공데이터 활용 창업경진대회 **우수상** |
| 2024.07 | 벤처스타트업 아카데미 산업체 문제해결 아이디어 경진대회 **우수상** |
| 2017.04 | 지방기능경기대회 웹디자인 및 개발 **우수상** |

<br>

---

<h3>🎓 Education</h3>

**영남대학교** 컴퓨터공학과 `2018.03 ~ 2025.02`

<br>

---

<h3>📊 GitHub Stats</h3>

<p>
  <img src="https://streak-stats.demolab.com?user=OhSSangHoon&theme=radical&hide_border=false" />
</p>
<p>
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=OhSSangHoon&theme=radical" />
</p>

<br>

---

<h3>📬 Contact</h3>

📧 fiveon555@gmail.com &nbsp;|&nbsp; 🐱 [github.com/OhSSangHoon](https://github.com/OhSSangHoon)
